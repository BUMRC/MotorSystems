...
class MotorNode(Node):
    ...
    def init_hardware(self):
        try:
            self.forward_pin = self.config['forward_pin']
            self.reverse_pin = self.config['reverse_pin']
    ...
    def velocity_callback(self, msg: Float64):
        self.target_velocity = msg.data #TODO: actually set this up for correct data
        self.last_command_time = self.get_clock().now()
...
def main():
    ...
    for motor_id in range(5):
        config = {
            ...
            'forward_pin_id': motor_id * 2 + (motor_id % 2) + 1,
            'reverse_pin_id': motor_id * 2 + 1 - (motor_id % 2) + 1,
